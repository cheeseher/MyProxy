...
## 📊 5.1 订阅信息面板 (Subscription Info)

在首页显示机场订阅的**已用流量**、**总流量**和**过期时间**。

1.  **安装模块**：
    ```text
    https://raw.githubusercontent.com/cheeseher/MyProxy/main/SubscriptionInfo.sgmodule
    ```
2.  **配置链接 (必做)**：
    *   安装后，首页会出现 "订阅信息 (需配置)" 面板。
    *   长按该面板 -> 点击 **"编辑"**。
    *   在 **"参数" (Argument)** 一栏中，粘贴您的**机场订阅链接**。
    *   点击 **"完成"**，数据即可刷出。
...
*   **2025-12-31 (v1.1.0)**
    *   ✅ **修复中银香港等银行 App 登录问题**: 强制 `skip-proxy` 直连 `*.bochk.com`, `*.za.group` 等域名 (IJ0050 错误修复)。
    *   ✅ **资源源优化**: 全面切换 `blackmatrix7` 规则为 **GitHub Raw** 源，彻底解决 jsDelivr 污染导致的更新超时。
    *   ✅ **文档修正**: 修复了同步指南中指向旧文件的错误链接。
    *   ✨ **新特性**: 正式加入 TikTok (2025 解锁/换区) 和 加密货币 (防风控) 专属策略组。

*   **2025-12-30 (v1.0.0)**
    *   🚀 **初始发布**: 极致 Surge 双端配置。
    *   ✨ **十大策略组**: 涵盖 AI、流媒体、办公、GitHub 加速等。
    *   ✨ **自动更新架构**: 推出分离式配置 (Detached Profile) 方案。
